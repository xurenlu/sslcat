...
func (gs *GitServer) loadApps() error {
	...
	for _, app := range gs.apps {
		...
		if !filepath.IsAbs(app.LogsDir) {
			// 先尝试基于工作目录的相对路径
			absPath := filepath.Join(workDir, app.LogsDir)
...
export GIT_CONFIG_VALUE_1="$BARE_REPO"
...
while read oldrev newrev refname; do
    ...
    # 注意：不使用 -L，避免跟随重定向导致 RemoteAddr 变成公网 IP
    DEPLOY_RESPONSE=$(timeout 10 curl -s --max-time 10 -X POST "$API_URL" \
