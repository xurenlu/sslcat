...
企业时间追踪系统，支持工时记录、项目时间统计、团队时间管理、工时报表等功能，类似 Toggl。
...
- **postgres 用户名**: {app_name}_time_tracking
- **postgres 密码**: 自动生成（18位随机密码）
- **postgres 数据库名**: {app_name}_time_tracking
- **redis 密码**: 自动生成（16位随机密码）
- **time-tracking Secret Key**: 自动生成（32位随机密钥）
...
- `time_tracking_data`: time-tracking 数据文件
...
1. 更多信息请访问：https://github.com/time-tracking
