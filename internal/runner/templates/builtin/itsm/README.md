...
专业的IT服务管理系统（ITSM），支持服务台、事件管理、变更管理、问题管理、配置管理（CMDB）等功能。
...
- **postgres 用户名**: {app_name}_itsm
- **postgres 密码**: 自动生成（18位随机密码）
- **postgres 数据库名**: {app_name}_itsm
- **redis 密码**: 自动生成（16位随机密码）
- **itsm Secret Key**: 自动生成（32位随机密钥）
...
1. 更多信息请访问：https://github.com/itsm
