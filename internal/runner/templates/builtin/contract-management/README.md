...
企业合同全生命周期管理系统，支持合同起草、合同审批、合同签署、合同执行、合同到期提醒等功能。
...
- **contract-management Secret Key**: 自动生成（32位随机密钥）
- **postgres 用户名**: {app_name}_contract
- **postgres 密码**: 自动生成（18位随机密码）
- **postgres 数据库名**: {app_name}_contract
...
- `contract_data`: contract-management 数据文件
- `contract_documents`: contract-management 数据存储
...
1. 更多信息请访问：https://github.com/contract-management
