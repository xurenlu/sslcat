...
供应商自助服务平台，支持供应商注册、订单查询、对账管理、发票上传、供应商评价等功能。
...
- **postgres 用户名**: {app_name}_supplier_portal
- **postgres 密码**: 自动生成（18位随机密码）
- **postgres 数据库名**: {app_name}_supplier_portal
- **redis 密码**: 自动生成（16位随机密码）
- **supplier-portal Secret Key**: 自动生成（32位随机密钥）
...
- `supplier_portal_data`: supplier-portal 数据文件
...
1. 更多信息请访问：https://github.com/supplier-portal
