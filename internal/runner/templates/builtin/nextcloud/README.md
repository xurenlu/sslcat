...
Nextcloud 是一个开源的自托管云存储和协作平台，类似 Dropbox/Google Drive，但完全私有化。
...
- **mysql 用户名**: {app_name}_nc
- **mysql 密码**: 自动生成（20位随机密码）
- **mysql 数据库名**: {app_name}_nextcloud
- **mysql Root 密码**: 自动生成（24位随机密码）
...
- `nextcloud_data`: nextcloud 数据存储
- `nextcloud_config`: nextcloud 配置文件
