...
企业内部审计管理系统，支持审计计划、审计执行、审计报告、问题跟踪、整改管理等功能。
...
- **postgres 用户名**: {app_name}_audit
- **postgres 密码**: 自动生成（18位随机密码）
- **postgres 数据库名**: {app_name}_audit
- **redis 密码**: 自动生成（16位随机密码）
- **audit-management Secret Key**: 自动生成（32位随机密钥）
...
- `audit_data`: audit-management 数据文件
...
1. 更多信息请访问：https://github.com/audit-management
