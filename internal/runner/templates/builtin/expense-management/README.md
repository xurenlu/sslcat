...
企业费用报销管理系统，支持费用申请、审批流程、发票上传、费用统计等功能。
...
- **postgres 用户名**: {app_name}_expense
- **postgres 密码**: 自动生成（18位随机密码）
- **postgres 数据库名**: {app_name}_expense
- **redis 密码**: 自动生成（16位随机密码）
- **expense-management Secret Key**: 自动生成（32位随机密钥）
...
- `expense_data`: expense-management 数据文件
- `expense_uploads`: expense-management 数据存储
...
1. 更多信息请访问：https://github.com/expense-management
