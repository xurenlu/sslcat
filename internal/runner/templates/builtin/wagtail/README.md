...
基于 Django 的企业级 CMS，适合构建复杂的内容管理系统。
...
- **postgres 用户名**: {app_name}_wagtail
- **postgres 密码**: 自动生成（18位随机密码）
- **postgres 数据库名**: {app_name}_wagtail
- **wagtail Secret Key**: 自动生成（50位随机密钥）
...
- `wagtail_data`: wagtail 数据存储
- `wagtail_static`: wagtail 数据存储
