...
```bash
# 1) Установка одним кликом (Linux)
# Для пользователей в континентальном Китае (ускорено через sslcat.com)
curl -fsSL https://sslcat.com/xurenlu/sslcat/main/scripts/install-from-release-zh.sh | sudo bash -s -- -v 1.0.11
# Пользователи вне континентального Китая могут использовать GitHub raw напрямую:
# curl -fsSL https://raw.githubusercontent.com/xurenlu/sslcat/main/scripts/install-from-release.sh | sudo bash -s -- -v 1.0.11

# 2) Быстрый тест на macOS локально (или скачать darwin пакет вручную)
...
# Первый вход: admin / admin*9527 (принудительно потребует смены пароля и создаст admin.pass)

# 3) Опционально: Запуск одним кликом через Docker Compose
docker compose up -d
```
...
```bash
# Скачать скрипт установки с GitHub
...
### Встроенное Развертывание (Один Файл)

```bash
# Создать встроенный пакет развертывания
./deploy-embedded.sh

# Или создать Linux версию
./deploy-embedded.sh linux

# Затем загрузить директорию deploy/ на сервер
```
