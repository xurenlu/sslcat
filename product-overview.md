...
SSLcat 是一个集成 SSL 证书管理、反向代理、WAF、安全审计、站点托管和 Git 部署能力的自托管网关管理工具。2.0.x rc 主线侧重稳定性，2.1.x 起新增 **MCP 内置接入**，让 AI 客户端可以直接担任运维助理；2.2.x 起补强 CLI 运维工具箱，让无 Web/无 AI 客户端的服务器现场也能完成基础诊断和站点管理；2.3.x 起增强 MCP 可观测性、证书冷启动可靠性和 Web 站点管理一致性，让 AI 能直接查看近期内部错误和所有站点 error log，并减少历史证书文件命名造成的 TLS 命中问题。
...
- 自定义端口模式的新安装默认使用 `18080`，避开常见业务端口 `8080`。
- DNS Provider 和证书挑战方法配置需要即时同步到 SSL 运行时，避免申请证书时错误回退 HTTP-01。
- 配置热重载采用显式字段检测加配置段兜底检测，未单独列出的配置变化也应刷新核心运行时。
- systemd `ExecReload` 通过 `SIGHUP` 触发进程内配置热重载，不关闭监听端口；只有 `SIGINT` / `SIGTERM` 进入优雅停机。
...
- `2.3.0-rc3`：Web 站点管理删除修复。静态/PHP 站点删除现在会真实调用后端 DELETE 接口并在成功后刷新列表；站点列表兼容 `root`/`index`/`fcgi_addr` 等后端字段，避免根目录和入口信息显示为空。
- `2.3.0-rc2`：证书冷启动匹配修复。TLS 握手在精确域名与 `*.父域` 文件都未命中时，会扫描磁盘证书 SAN，支持加载“具体域名文件名中保存的通配符证书”，避免重启后新子域临时找不到证书。
