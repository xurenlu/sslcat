...
SSLcat 是一个集成 SSL 证书管理、反向代理、WAF、安全审计、站点托管和 Git 部署能力的自托管网关管理工具。2.0.x rc 主线侧重稳定性，2.1.x 起新增 **MCP 内置接入**，让 AI 客户端可以直接担任运维助理；2.2.x 起补强 CLI 运维工具箱，让无 Web/无 AI 客户端的服务器现场也能完成基础诊断和站点管理；2.3.x 起增强 MCP 可观测性与证书冷启动可靠性，让 AI 能直接查看近期内部错误和所有站点 error log，并减少历史证书文件命名造成的 TLS 命中问题。
