...
// 提取接口定义（从 export interface Translation 到第一个 export const）
const interfaceMatch = content.match(/export interface Translation \{[\s\S]*?\n\}/);
if (!interfaceMatch) {
  console.error('Could not find Translation interface');
  process.exit(1);
}
...
`;
    ...
    console.error(`Could not find translation for ${lang.name}`);
  }
});
