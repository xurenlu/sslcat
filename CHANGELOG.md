...
- **CI 前端构建稳定性**：固定 GitHub Actions 使用 Yarn 4.15.0，并将前端安装改为 `yarn install --immutable`；同步升级 `frontend/yarn.lock` 元数据，避免 `yarn@stable` 漂移导致锁文件在 CI 中被拒绝修改。
...
- **证书申请长连接稳定性**：证书申请/重试 SSE 清除全局写超时、增加心跳，并避免在 HTTP/2 响应里写入逐跳 `Connection` 头，降低 HTTP/2/HTTP/3 场景下进度流被提前切断的风险。
- **WebSocket 高并发退出路径**：优化 WebSocket 代理错误上报改为非阻塞，退出时不再关闭仍可能被 goroutine 使用的内部通道，降低连接风暴下 goroutine 卡住或 panic 的风险。
- **同域证书申请并发保护**：同一域名的证书申请和进度通道改为独占，防止并发申请互相覆盖通道、误关新请求或触发已关闭通道发送 panic。
- **证书文件落盘可靠性**：证书、私钥、管理员密码和恢复码写入改为 fsync 后原子替换，降低崩溃或磁盘抖动导致半写入文件的风险。
- **证书 API 输入校验**：证书生成、重试、删除、预检和上传接口统一校验域名，避免通过异常域名触发路径穿越或无效证书操作。

## [2.0.0-rc22] - 2026-05-19

### ✨ 新增

- **管理员恢复码重置密码**：启动时自动确保存在一次性管理员恢复码文件，忘记密码时可通过恢复码重置管理员密码；恢复码验证成功后旧码立即失效并生成新码。

### 🐛 Bug 修复

- **管理员密码写入可靠性**：管理员密码与恢复码文件改为临时文件加原子替换写入，降低进程中断或磁盘抖动导致敏感文件半写入的风险。

## [2.0.0-rc21] - 2026-05-19

### 🐛 Bug 修复

- **封禁文件热生效**：安全管理器会定时监控 `sslcat.block` 与同目录 `whitelist.json` 的变更，删除或编辑文件后自动重载内存封禁/白名单状态，手工解封不再需要重启服务。

## [2.0.0-rc20] - 2026-05-19

### 🐛 Bug 修复

- **后台组件停止安全**：CDN processing 去重通知、SimpleCache、错误日志限流器、安全管理器和高级限流器的关闭路径增加幂等保护，降低热重载、重复 Stop/Close 或超时清理并发触发时的 panic 风险。
...
- **修复 Git 仓库权限问题**：
  - 修复 `initGitRepo` 中 `chown -R` 影响父目录的问题
  - 只 `chown` 应用自己的目录，不影响其他应用
  - 修复 git push 时 "unable to create temporary object directory" 错误
  - 防止 `/opt/sslcat/data/runners/git/` 目录权限被修改

## [1.7.7-rc1] - 2026-03-19

### 🐛 Bug 修复

- **添加全局部署互斥锁**：
  - 确保同时只有一个应用在部署，避免资源竞争和死锁
  - 解决 `GetApp` 可能被长时间阻塞的问题
  - 添加部署锁的获取和释放日志（🔒/🔓）
  - 防止多个部署同时修改 Docker、端口等资源

### 🔧 改进

- **部署并发控制**：
  - 在 `processGitPushWithRecord` 中添加全局部署锁
  - 记录当前正在部署的应用名称
  - 部署完成后自动释放锁
...
- **修复 Security Manager 的持久化竞争窗口与文件原子性问题**：
  - 将封禁列表与白名单持久化改为队列化串行写入，避免并发写盘导致旧快照覆盖新状态
  - 写盘流程改为临时文件 + `fsync` + `rename` 原子替换，避免异常中断时出现半写入文件
  - `Stop` 阶段补充持久化 worker 收敛，尽量确保最后状态落盘
- **补充并发一致性测试**：
  - 新增封禁列表/白名单“内存状态与落盘状态一致”回归测试
...
- **国际化补全**：ClusterSettings Service Mesh、SecurityMTLS、EdgeRouting、ThreatIntel 等页面剩余硬编码中文全部改为 i18n，补全 zh-CN、en-US 翻译
...
- **install-sslcat.sh 静默升级模式**：
  - 支持 `--silent-upgrade` 或 `-s` 参数
  - 安装二进制、systemctl 重启 sslcat，不覆盖配置文件（sslcat.conf、git-hook.conf）
  - 无交互，适合 CI/cron 自动升级
