...
func main() {
	...
	testGetUsers(client, baseURL)

	fmt.Println("")

	// 测试 2: 添加用户
	fmt.Println("🔵 测试 2: 添加用户")
	testAddUser(client, baseURL)
...
func testGetUsers(client *http.Client, baseURL string) {
	start := time.Now()
...
func testAddUser(client *http.Client, baseURL string) {
	start := time.Now()
	...
	req, err := http.NewRequest("POST", baseURL+"/api/users", strings.NewReader(string(jsonData)))
	if err != nil {
		fmt.Printf("❌ 创建请求失败: %v\n", err)
		return
	}
