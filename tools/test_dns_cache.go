...
func main() {
	...
	testDNSProvidersAPI(baseURL)

	fmt.Println("")

	// 测试 2: 手动刷新缓存
	fmt.Println("🔵 测试 2: 手动刷新 DNS 缓存")
	testDNSRefreshAPI(baseURL)
...
func testDNSProvidersAPI(baseURL string) {
	start := time.Now()

	resp, err := http.Get(baseURL + "/api/dns/providers")
	if err != nil {
		fmt.Printf("❌ 请求失败: %v\n", err)
		return
	}
...
func testDNSRefreshAPI(baseURL string) {
	start := time.Now()

	resp, err := http.Post(baseURL+"/api/dns/refresh", "application/json", nil)
	if err != nil {
		fmt.Printf("❌ 请求失败: %v\n", err)
		return
	}
